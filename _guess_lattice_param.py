...
def get_structure_with_guessed_lattice_params(structure):
    ...
    min_dist_list = []
    for combo in combos:
        cur_nns = nn_info_df[(nn_info_df["center_indices"] == combo[0]) & (nn_info_df["points_indices"] == combo[1])]
        # print(cur_nns)
        ...
        min_dist_list.append(symbol_list + [min_dist_for_site_pair] + [min_dist_predicted_for_site_pair] + [scale_ratio_implied])
    scale_ratio = np.max(pd.DataFrame(min_dist_list).iloc[:,-1]) * general_multiplication_factor
    structure_with_guessed_lattice_param = copy.deepcopy(structure)
    structure_with_guessed_lattice_param.scale_lattice(scale_ratio**3 * structure.volume)
