...
def featurize_structures_in_folder(folder_path):
    ...
    for struct_filepath in struct_filepaths:
        ...
        cur_descriptor = np.array(
            m3gnet_matgl.model.featurize_structure(struct, output_layers=["readout"])[
                "readout"
            ].detach()
        )[0]
        eform = m3gnet_matgl.model.predict_structure(
            struct
        ).item()  # the .item() gets the value from the torch.tensor object
    ...
    with open(
        f"{candidates_path}/{Path(folder_path).name}_descriptors.json", "w"
    ) as fout:
