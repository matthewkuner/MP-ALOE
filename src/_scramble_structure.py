...
def generate_elastic_deformations(
    ...
    min_z_val = -2
    max_z_val = 2
    strain_magnitudes = truncnorm.rvs(
        min_z_val,
        max_z_val,
        size=len(strain_states),
        scale=stdev_of_normal_distribution,
    )

    strains = []
    for i in range(0, len(strain_states)):
        strains.extend(
            [Strain.from_voigt(strain_magnitudes[i] * np.array(strain_states[i]))]
        )
...
def scramble_structure(structure):
    ...
    stdev = 0.020 / len(
        structure
    )  # set to 0.005/len(structure) for 1st active learning iteration
    min_z_val = -3
    max_z_val = 3
    for i in range(0, len(structure)):
        random_translation_mag = truncnorm.rvs(
            min_z_val, max_z_val, size=3, scale=stdev
        )
    ...
    deformations = generate_elastic_deformations(
        stdev_of_normal_distribution=0.02
    )  # set to 0.02 for 1st active learning iteration
