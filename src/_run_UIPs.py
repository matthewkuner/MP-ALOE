...
from _paths import data_path, models_path
...
mace_calculator_1 = MACECalculator(
    model_paths=models_path / "mace_iter_2_model_1-cpu.model",  # insert path to model 1
    default_dtype="float32",
    device="cpu",
)

mace_calculator_2 = MACECalculator(
    model_paths=models_path / "mace_iter_2_model_2-cpu.model",  # insert path to model 2
    default_dtype="float32",
    device="cpu",
)

mace_calculator_3 = MACECalculator(
    model_paths=models_path / "mace_iter_2_model_3-cpu.model",  # insert path to model 3
    default_dtype="float32",
    device="cpu",
)
...
mace_elemental_energy_filepath = data_path / "formation_energies_mace_avg_iter_2.json"
with open(mace_elemental_energy_filepath, "rt") as f:
    mace_avg_elemental_energies = json.load(f)
    ...
    for i in range(0, len(calculators)):
        ...
        if convert_to_formation_energies:
            cur_energy = calculate_formation_energy(
                cur_energy, structure, mace_avg_elemental_energies
        ...
        results.append(
            {"energy": cur_energy, "forces": cur_forces, "stress": cur_stress}
        )
